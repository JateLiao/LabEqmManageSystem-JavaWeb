...
var managerId = getUrlPram('managerId');
var managerName = getUrlPram('managerName');
...
function loadEquipmentList(title){
	eqmWin = $('#eqmList').datagrid({
		...
		columns : [ [
		             ...
		             {field : 'propertyNo',title : '设备资产号',width : '10%',align : 'center', sortable:true},
	...
	$("#eqmList").datagrid('setTitle','我的托管设备');
} 

/*我的设备初始化*/
function initMyEqmList(){
	if (isNullOrEmpty(managerId) || isNullOrEmpty(managerName)) {
		loadEquipmentList('所有设备');
		return;
	}
...
function addOrModifyEquipmentWindow(row) { 
	...
	if (isModify) { //修改
		...
		$("#propertyNo").attr("class","eqm_update_propertyNo");
		$('#propertyNo').addClass("eqm_update_propertyNo");
...
function addOrModifyEquipment(){
	...
	if(isModify){ //修改
		url = "../../equipments/updateEquipment.htm?param=" + param;
	} else{ //添加
		if (isNullOrEmpty(managerId) || isNullOrEmpty(managerName)) {
			$('#manager').textbox("setValue", "");
			$('#managerId').textbox("setValue", "");
		}
		url = "../../equipments/addEquipment.htm?param=" + param;
	}
	$.ajax({
		type: 'POST',
		url: url,
...
function initModify(row){
	...
	$('#propertyNo').val(row[0].propertyNo);
...
function expotEquipmentWindow(){
	parent.layer.open({
		...
		maxmin : true, // 开启最大化最小化按钮
		area : [ '440px', '280px' ],
		content : 'equipment/exportEquipment.html'
	});
}
	...
	parent.layer.open({
	    ...
	    yes: function(index, layero){ //或者使用btn1
	    	...
	    	$.ajax({
				type : "POST",
				dataType : "JSON",
				url : "../../equipments/delete.htm?param=" + param,
				success : function(data) { 
					$('#eqmList').datagrid('reload');
